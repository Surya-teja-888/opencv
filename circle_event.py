...
def draw_circle(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDBLCLK:
        ...
        cv2.circle(image,(x,y),50,colors["light_gray"])
...
image = np.ones((600, 600, 3), dtype="uint8")

# We create a named window where the mouse callback will be established:
cv2.namedWindow('Image mouse')


cv2.setMouseCallback('Image mouse', draw_circle) # this is callback function to "draw_circle"
